  ...
  return {
    ...
    version(version, bumps) {
      return [
        ...
        version.major < this.major || version.minor < this.minor ?  0 : Math.max(0, version.patch) + bumps
      ].join(".");
    },
    asString() {
...
const versionSpec = async( version_spec_input ) => {
  if (/[0-9*]+\.[0-9*]+\.[0-9*]+/.test(version_spec_input)) {
      ...
      const content = await fs.promises.readFile(path.join(process.env.GITHUB_WORKSPACE, version_spec_input), { encoding: "utf8"});
      const row = content.split(/\\[nrt]/g).find(line => line.startsWith("version"));
