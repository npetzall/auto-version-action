...
describe("version_spec", () => {
  test("input is version_spec", async () => {
    ...
    expect(version_spec.version({ major: 1, minor: 2, patch: 3 }, 4)).toBe(
      "1.2.7"
    );
  });
  test("input is file", async () => {
    const version_spec = await versionSpec(
      path.join(".jest", "version.test.properties")
    );
    expect(version_spec.version({ major: 0, minor: 2, patch: 3 }, 4)).toBe(
      "0.3.0"
    );
  });
  ...
  test("spec is equal to version", async () => {
    ...
    expect(version_spec.version({ major: 0, minor: 3, patch: 3 }, 4)).toBe(
      "0.3.7"
    );
  });
